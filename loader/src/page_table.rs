...
use x86_64::structures::paging::{
    FrameAllocator, OffsetPageTable, Mapper, PageSize, PageTable, PageTableFlags, PhysFrame
};
...
impl KernelPageTable {
    ...
    pub fn page_table_frame(&self) -> PhysFrame {
        PhysFrame::from_start_address(self.page_table_start).expect("Kernel PML4 location is not page-aligned")
    }

    /// Maps a range of physical frames to pages in the kernel address space
    pub fn map(&mut self, boot_services: &BootServices, pages: PageRange, frames: PhysFrameRange, flags: PageTableFlags) {
        assert_eq!(pages.end - pages.start, frames.end - frames.start, "Physical and virtual ranges are not the same size");

        trace!("Mapping {} pages starting at {:?} to {:?} with {:?}", pages.end - pages.start, pages.start, frames.start, flags);
        ...
        for (page, frame) in pages.zip(frames) {
            unsafe {
                mapped_table.map_to(page, frame, flags, &mut allocator)
...
unsafe impl <'a, S: PageSize> FrameAllocator<S> for UefiFrameAllocator<'a> {
    fn allocate_frame(&mut self) -> Option<PhysFrame<S>> {
        ...
        .ok()
        .map(|start_addr| {
            PhysFrame::from_start_address(PhysAddr::new(start_addr)).expect("UEFI boot services returned an unaligned frame")
        })
    }
}