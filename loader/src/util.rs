...
pub fn allocate_frames(system_table: &SystemTable<Boot>, count: usize, typ: MemoryType) -> (PhysAddr, &'static mut [u8]) {
    let phys_addr = match system_table.boot_services().allocate_pages(AllocateType::AnyPages, typ, count)
        .log_warning() {
            Ok(addr) => addr,
    ...
    let buf = unsafe {
        slice::from_raw_parts_mut(phys_addr as *mut u8, count * PAGE_SIZE as usize)
    };
