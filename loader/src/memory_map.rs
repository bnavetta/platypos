...
pub fn map_uefi_environment(page_table: &mut KernelPageTable, boot_services: &BootServices) {
    ...
    let (_, memory_map) = boot_services
        .memory_map(&mut buf)
        .expect_success("Could not get UEFI memory map");

    let regions = memory_map.filter(|desc| {
        desc.att.contains(MemoryAttribute::RUNTIME)
            || match desc.ty {
    ...
    for region in regions {
        ...
        let pages = Page::containing_address(VirtAddr::new(region.phys_start))
            .range_to(region.page_count as usize);
        let frames = PhysFrame::containing_address(PhysAddr::new(region.phys_start))
            .range_to(region.page_count as usize);
...
pub fn create_kernel_stack(page_table: &mut KernelPageTable, boot_services: &BootServices) {
    ...
    let pages = Page::from_start_address(KERNEL_STACK_START)
        .unwrap()
        .range_to(KERNEL_STACK_PAGES);
    let frames = PhysFrame::from_start_address(PhysAddr::new(phys_start))
        .unwrap()
        .range_to(KERNEL_STACK_PAGES);
    page_table.map(
        boot_services,
    ...
    debug!(
        "Allocated {}-page kernel stack at {:#x}",
        KERNEL_STACK_PAGES, phys_start
    );
}
...
pub fn create_boot_info(
    ...
    let pages = Page::from_start_address(BOOT_INFO_ADDRESS)
        .unwrap()
        .range_to(1);
    let frames = PhysFrame::from_start_address(PhysAddr::new(phys_addr))
        .unwrap()
        .range_to(1);
    page_table.map(
        boot_services,
